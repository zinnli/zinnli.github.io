...
- 이전에 스터디 때 공부해보긴 했는데 사실 직접 사용을 안해봤다보니 그냥 멀게 느껴져 따로 만져봐야 이해가 될 것 같다는 생각이 들어 프리온보딩에서 next 강의 들은 김에 위 프로젝트를 진행했다. (아직 수정할 게 많아서 틈틈히 추가해야겠지만.. 배포도 제대로 해본 적이 없어서 배포까지가 목표 / 배포까지 완료되면 next 블로그 만들었던 과정 정리 예정)
