...
> ### react에서 리덕스로 <br/>` (dispatch) -> action -> reducer -> store -> 리덕스에서 리액트로 값 전달 (useSelector)`
...
3. type 사용 : 액션 객체를 리듀서로 보낼 때 -> 리뷰서는 객체 안의 type이라는 key로 수정할 부분을 정함
4. payload : 전달값
...
2. state.number (기존의 nubmer)에 action.paylaod(유저가 더하길 원하는 값)을 더한다.
...
이틀동안 이해를 제대로 못해서 큰일 났다 싶었는데 오늘 기술 매니저님의 전체적인 설명으로 대략 흐름은 알게 된 것 같다. <br/>뭔가 열심히 듣고 열심히 정리했는데 이게 맞는지는 잘 ㅎ.. 공부하면서 하나씩 추가+수정해나가야겠다.
