...
-    현재 호출된 함수가 선언되는 시점에서의 LexicalEnvironment를 참조하는 포인터
-    연결리스트의 형태를 띄며 '선언 시점의 LexicalEnvironment(=해당 함수가 속한 상위 스코프의 범위)'를 찾아 올라감
...
-    자바스크립트에서 실행 컨텍스트란, **`코드를 실행하기 위해 필요한 정보들을 가진 범위`**를 객체 형태로 나타낸 것
